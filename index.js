...
async function fetchUserInfo(token, proxy, context) {
  ...
  try {
    const response = await requestWithRetry('get', 'https://api.asterai.xyz/client/user', null, getAxiosConfig(proxy, token), 3, 2000, context);
...
async function fetchTasks(token, proxy, context) {
  try {
    const response = await requestWithRetry('get', 'https://api.asterai.xyz/client/tasks', null, getAxiosConfig(proxy, token), 3, 2000, context);
...
async function fetchCompletedTasks(token, proxy, context) {
  try {
    const response = await requestWithRetry('get', 'https://api.asterai.xyz/client/user-tasks/completed', null, getAxiosConfig(proxy, token), 3, 2000, context);
...
async function verifyTask(token, taskId, taskName, taskCategory, proxy, context) {
  ...
  try {
    while (retryCount < maxRetries) {
      const response = await requestWithRetry('post', `https://api.asterai.xyz/client/user-tasks/verify/${taskId}`, {}, getAxiosConfig(proxy, token), 3, 2000, taskContext);
...
async function claimTask(token, taskId, taskName, taskCategory, proxy, context, maxRetries = 5) {
  ...
  try {
    while (retryCount < maxRetries) {
      const response = await requestWithRetry('post', `https://api.asterai.xyz/client/user-tasks/claim/${taskId}`, {}, getAxiosConfig(proxy, token), 3, 2000, taskContext);
...
async function fetchStatistics(token, proxy, context) {
  ...
  try {
    const userResponse = await requestWithRetry('get', 'https://api.asterai.xyz/client/user', null, getAxiosConfig(proxy, token), 3, 2000, context);
    const rewardsResponse = await requestWithRetry('get', 'https://api.asterai.xyz/client/rewards/statistic', null, getAxiosConfig(proxy, token), 3, 2000, context);
    ...
    return {
      ...
      socialAst: rewardsData.social || 0,
      refAst: rewardsData.ref || 0,
      totalAst: (rewardsData.social || 0) + (rewardsData.ref || 0),
...
async function processAccount(token, index, total, proxy = null) {
  ...
  printInfo('Social AST', stats.socialAst, context);
  printInfo('Referral AST', stats.refAst, context);
...
async function run() {
  ...
  console.log(gradient.retro(centerText('=== FOLLOW TIKTOK 🚀 : AirdropRefferal (@Souiy1) ===', terminalWidth)));
  console.log(gradient.retro(centerText('✪ AUTO COMPLETE DAILY & SOCIAL TASKS ✪', terminalWidth)));
console.log(gradient.retro(centerText('✪ Satukan Solidaritas Bantu Palestina! ✪', terminalWidth)));
console.log(gradient.retro(centerText('✪ Link Donasi: https://digital.dompetdhuafa.org/donasi/jagapalestina ✪', terminalWidth)));
